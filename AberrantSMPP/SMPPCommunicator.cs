...
using Aberrant.SMPP.Core.EventObject;
using Aberrant.SMPP.Core.Exceptions;
...
namespace Aberrant.SMPP.Core
{
    ...
    public class SMPPCommunicator : Component, IDisposable
    {
        ...
        /// </summary>
        public bool SentUnbindPacket { get; private set; } = true;

        /// <summary>
        /// The binding type(receiver, transmitter, or transceiver)to use 
        /// when connecting to the SMSC.
        /// </summary>
        public SmppBind.BindingType BindType { get; set; }
        ...
        /// </summary>
        public Pdu.NpiType NpiType { get; set; }

        /// <summary>
        /// The type of number that this SMPPCommunicator should use.  
        /// </summary>
        public Pdu.TonType TonType { get; set; }

        /// <summary>
        /// The SMPP specification version to use.
        /// </summary>
        public Pdu.SmppVersionType Version { get; set; }
        ...
        public bool Connected => asClient != null && asClient.Connected;
        ...
        public bool Bound
        {
            get
            {
                lock (_bindingLock) return _Bound;
            }
        }
        ...
        public SmppResponse SendRequest(SmppRequest request)
        {
            ...
            var signalled = state.EventHandler.WaitOne(ResponseTimeout);
        ...
        public IEnumerable<SmppResponse> SendRequests(IEnumerable<SmppRequest> requests)
        {
            ...
            if (Thread.CurrentThread.GetApartmentState() == ApartmentState.STA)
            {
                ...
                {
                    if (WaitHandle.WaitAny(new[] { handler }, ResponseTimeout) == 258)
                    {
                        break;
                    }
                    else
            ...
            {
                signalled = WaitHandle.WaitAll(handlers, ResponseTimeout * handlers.Length);
            }
                    ...
                    if (packet is SmppResponse || packet is SmppGenericNack)
                    {
                        lock (_RequestsAwaitingResponse)
                        {
                            if (_RequestsAwaitingResponse.ContainsKey(packet.SequenceNumber))
                            {
                                ...
                                state.Response = packet is SmppGenericNack ? new SmppGenericNackResp(packet.PacketBytes) : packet as SmppResponse;
        ...
        private void FireEvents(Pdu response)
        {
            ...
            else if (response is SmppSubmitSm)
            {
                ...
                else
                {
                    ...
                    pdu.MessageId = Guid.NewGuid().ToString().Substring(0, 10);
                    pdu.CommandStatus = 0;
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            Helper.ShallowExceptions(() =>
            {
                if (asClient != null) asClient.Dispose();
            });
